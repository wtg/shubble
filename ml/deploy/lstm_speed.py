...
def load_lstm_velocity(
    ...
    """
    Load all trained LSTM models for velocity predictions from cache.
    ...
    Args:
        ...
        output_columns: Target columns predicted by the model (default: ['next_speed_kmh'])
    ...
    Example:
        >>> # Load all trained LSTM models for velocity prediction
        >>> models = load_lstm_velocity()
...
def load_lstm_velocity_for_route(
    route_name: str,
    polyline_idx: int,
    input_columns: list[str] = ['latitude', 'longitude', 'speed_kmh'],
    output_columns: list[str] = ['next_speed_kmh'],
    ...
    Example:
        ...
        >>> model = load_lstm_velocity_for_route('East Route', 0)
        >>> prediction = model.predict(input_sequence)
    """
    from ml.models.lstm import LSTMModel
...
def list_cached_velocity_models() -> list[dict]:
    """
    List all cached LSTM models for velocity prediction with metadata.
    ...
    Example:
        >>> models = list_cached_velocity_models()
        >>> for model in models:
